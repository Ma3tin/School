...
public class FOUR {
    public static void main(String[] args) {
        ...
        for (int i = 0; i < pole.length; i++) {
            pas = pole[i];
            pas = pas.replaceAll("\n", " ") + " ";
            /*String[] vals = pas.split(" ");
            System.out.println();
            for (int j = 0; j < vals.length; j++) {
                String temp = vals[j];
                end = temp.substring(4);
                next[j] = end;
            ...
            if (pole[i].contains("byr") && pole[i].contains("iyr") && pole[i].contains("eyr") && pole[i].contains("hgt") &&
                    pole[i].contains("hcl") && pole[i].contains("ecl") && pole[i].contains("pid")) {
                all++;
            }*/
            boolean valHgt = validateHeight(pas);
            boolean valByr = validateBirthYear(pas);
            boolean valiyr = validateIssueYear(pas);
            boolean valeyr = validateExpirationYear(pas);
            boolean valhcl = validateHairColor(pas);
            boolean valecl = validateEyeColor(pas);
            boolean valpid = validatePassportId(pas);
            if (valHgt && valByr && valiyr && valeyr && valhcl && valecl && valpid) {
                all++;
            }
        }
    ...
    protected static boolean validateBirthYear(String passport) {
        int ix = passport.indexOf("byr:");
        if (ix < 0) return false;
        int endIx = passport.indexOf(" ", ix);
        String birthStr = passport.substring(ix + 4, endIx);
        int birthYear = Integer.parseInt(birthStr);
        return (birthYear >= 1920 && birthYear <= 2002);
    }

    protected static boolean validateIssueYear(String passport) {
        int ix = passport.indexOf("iyr:");
        if (ix < 0) return false;
        int endIx = passport.indexOf(" ", ix);
        String issueStr = passport.substring(ix + 4, endIx);
        int issueYear = Integer.parseInt(issueStr);
        return (issueYear >= 2010 && issueYear <= 2020);
    }

    protected static boolean validateExpirationYear(String passport) {
        int ix = passport.indexOf("eyr:");
        if (ix < 0) return false;
        int endIx = passport.indexOf(" ", ix);
        String expStr = passport.substring(ix + 4, endIx);
        int expirationYear = Integer.parseInt(expStr);
        return (expirationYear >= 2020 && expirationYear <= 2030);
    }

    protected static boolean validateHeight(String passport) {
        try {
            int ix = passport.indexOf("hgt:");
            if (ix < 0) return false;
            int endIx = passport.indexOf(" ", ix);
            String heightStr = passport.substring(ix + 4, endIx);
    ...
    protected static boolean validateHairColor(String passport) {
        try {
            int ix = passport.indexOf("hcl:#");
            if (ix < 0) return false;
            int endIx = passport.indexOf(" ", ix);
            String hairColor = passport.substring(ix + 5, endIx);
            int birthYear = Integer.parseInt(hairColor, 16);
            return (hairColor.length() == 6);
        } catch (NumberFormatException e) {
            return false;
        }
    }

    protected static boolean validateEyeColor(String passport) {
        int ix = passport.indexOf("ecl:");
        if (ix < 0) return false;
        int endIx = passport.indexOf(" ", ix);
        String eyeColor = passport.substring(ix + 4, endIx);
        String[] eyeColors = {"amb", "blu", "brn", "gry", "grn", "hzl", "oth"};
        for (int i = 0; i < eyeColors.length; i++) {
            if (eyeColors[i].equals(eyeColor))
                return true;
        }
        return false;
    }

    protected static boolean validatePassportId(String passport) {
        try {
            int ix = passport.indexOf("pid:");
            if (ix < 0) return false;
            int endIx = passport.indexOf(" ", ix);
            String countryStr = passport.substring(ix + 4, endIx);
            int countryId = Integer.parseInt(countryStr);
            return (countryStr.length() == 9);
        } catch (NumberFormatException e) {
            return false;
        }
    }
